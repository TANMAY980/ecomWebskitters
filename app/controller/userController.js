...
class User {
  ...
  async Signin(req, res) {
    try {
      ...
      if (user.role === "admin") {
        ...
        const loginEmail = await sendEmail.SendSiginingMessage(
          req,
          res,
          user,
          url
  ...
  async VerifyMail(req, res) {
    try {
      ...
      if (!emailverification) {
        if (!existuser.is_verified) {
          const sendotp = await sendEmail.SendEmailVerificationOTP(
            req,
            res,
            existuser
          );
  ...
  async UserRegister(req, res) {
    try {
      const { email, name, username, password, confirm_password, phonenumber } =
        req.body;
      if (
        !(
          email &&
      ...
      const sendmail = await sendEmail.SendRegistrationEmail(
        req,
        res,
        user,
        user_password,
  ...
  async UpdatePasskey(req, res) {
    try {
      ...
      if (user) {
        ...
        const sendupdate = await sendEmail.SendUpdatePasswordMessage(
          req,
          res,
          user
        );
  ...
  async AddInCart(req, res) {
    try {
      ...
      await cart.save();
      const cartWithTotal = await cartmodel.aggregate([
        ...
        },
        {
          $addFields: {
            ...
            platformFee: 20,
            total: {
              $add: ["$subtotal", 25, 25, 20],
            },
          },
        },
      ]);
      if (cartWithTotal.length > 0) {
        await cartmodel.updateOne(
          { _id: cartWithTotal[0]._id },
          { $set: { total: cartWithTotal[0].total } }
        );
      }
  ...
  /******* buy now button function in home page********/
  ...
  async bynow(req, res) {
    try {
      ...
      await ordermodel.insertMany(cartData);
      await cartmodel.deleteOne({ user: userId });
      req.flash("success_msg", "Order placed successfully!");
      const order = await sendEmail.SendOrderNotification(req, res, user, url);
  ...
  async UpdateAddress(req, res) {
    try {
      ...
      const useraddress = await usermodel.findByIdAndUpdate(userid, {
        address,
      });
      if (useraddress) {
        return res.redirect("/addcart");
  ...
  async ForgotPasskey(req, res) {
    try {
      ...
      const token = jwt.sign({ userID: exisituser._id }, secret, {
        expiresIn: "5m",
      });
